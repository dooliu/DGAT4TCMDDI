...
from utils import init_hyper_space
...
def cal_predict_res(model, data_loader):
    ...
    # print(f"predict:{predict_y}, ground_truth:{real_y}")
    # real_y = real_y.cpu().clone().detach().numpy()
...
def bayesian_optimization(args, train_set, val_set, test_set):
    ...
    def objective(hyperparams):
        ...
        print(f"current best res:{trial_path}-{val_metric_to_minimize}------------")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c_1', '--csv-path-train', type=str, required=True,
                        help='Path to a csv file for loading a dataset')
    parser.add_argument('-c_2', '--csv-path-test', type=str, required=True,
                        help='Path to a csv file for loading a dataset')
    ...
    train_dataset = load_dataset(args, df_train)
    df_test = pd.read_csv(args['csv_path_test'])
    mkdir_p(args['result_path'])
    test_dataset = load_dataset(args, df_test)
    ...
    for _ in range(args['run_evals']):
        ...
        if args['num_evals'] is not None:
            ...
            print(f"best res path:{trial_path}")
        else:
            print('Use the manually specified hyperparameters')
            exp_config = get_configure(args['model']+"-cross")
    ...
    acc_list = [sum(x) / len(x) for x in chunked(metrics_map['ACC'], folds)]
    sn_list = [sum(x) / len(x) for x in chunked(metrics_map['SN'], folds)]
    sp_list = [sum(x) / len(x) for x in chunked(metrics_map['SP'], folds)]
    mcc_list = [sum(x) / len(x) for x in chunked(metrics_map['MCC'], folds)]
    precision_list = [sum(x) / len(x) for x in chunked(metrics_map['precision'], folds)]
    recall_list = [sum(x) / len(x) for x in chunked(metrics_map['recall'], folds)]
    r2_list = [sum(x) / len(x) for x in chunked(metrics_map['r2'], folds)]
    f1_list = [sum(x) / len(x) for x in chunked(metrics_map['f1'], folds)]
    auc_list = [sum(x) / len(x) for x in chunked(metrics_map['AUC'], folds)]
