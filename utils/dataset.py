...
from dgllife.utils.mol_to_graph import ToGraph, SMILESToBigraph

class MolReactionCSVDataset(object):
    """MoleculeCSVDataset
    ...
    def __init__(self, df, smiles_to_graph=None, node_featurizer=None, edge_featurizer=None,
                 smiles_column_a=None, smiles_column_b=None, cache_file_path=None, task_names=None, load=False,
                 log_every=1000, init_mask=True, n_jobs=1, error_log=None):
        self.df = df
        self.smiles = self.df[[smiles_column_a, smiles_column_b]].values
        if task_names is None:
            self.task_names = self.df.columns.drop([smiles_column_a, smiles_column_b]).tolist()
        else:
    ...
    def _pre_process(self, smiles_to_graph, load, log_every, init_mask, n_jobs, error_log):
        ...
        * Set missing labels to be 0 and use a binary masking
        ...
        if os.path.exists(self.cache_file_path) and load:
            ...
            graphs, label_dict = load_graphs(self.cache_file_path)
            self.graphs = np.array(graphs).reshape((-1, 2))
            ...
            if n_jobs > 1:
                ...
                for i, s in enumerate(self.smiles):
                    ...
                    tmp_graph_list = []
                    for tmp_S in s:
                        tmp_graph_list.append(smiles_to_graph(tmp_S))
                    self.graphs.append(tmp_graph_list)
            ...
            if init_mask:
                ...
                save_graphs(self.cache_file_path, np.array(self.graphs).flatten().tolist(),
